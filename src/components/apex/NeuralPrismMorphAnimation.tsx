...
type Phase = 'idle' | 'morphing_to_neural_prism' | 'showing_neural_prism'

function generateNeuralPrismWireframe(maxLines?: number): THREE.Vector3[][] {
  ...
  for (let i = 0; i < targetCount; i++) {
    const edge = NEURAL_PRISM_WIREFRAME_DATA[i]
...
function SculptureLines() {
  ...
  const targetLines = useMemo(() => generateNeuralPrismWireframe(MAX_LINE_COUNT), [])
  ...
  useFrame((state) => {
    ...
    if (phase === 'idle') {
      ...
      if (phaseElapsed > ANIMATION_CONFIG.timing.idleDuration) {
        ...
        setPhase('morphing_to_neural_prism')
        phaseStartRef.current = now
        morphProgressRef.current = 0
      }
    } else if (phase === 'morphing_to_neural_prism') {
      tesseractTimeRef.current += 0.016
    ...
    } else if (phase === 'showing_neural_prism') {
      // Stay showing neural prism - no transition back
    }
    
    const tesseractTime = tesseractTimeRef.current
    
    const tesseractColor = new THREE.Color(ANIMATION_CONFIG.colors.tesseract)
    const neuralPrismColor = new THREE.Color(ANIMATION_CONFIG.colors.neuralPrism)
    
    let targetColor: THREE.Color
    if (phase === 'idle') {
      targetColor = tesseractColor
    } else if (phase === 'morphing_to_neural_prism') {
      targetColor = tesseractColor.clone().lerp(neuralPrismColor, morphProgressRef.current)
    } else {
      targetColor = neuralPrismColor
    }
  ...
  const lineObjects = useMemo(() => {
    ...
    for (let i = 0; i < MAX_LINE_COUNT; i++) {
      ...
      const material = new THREE.LineBasicMaterial({
        color: ANIMATION_CONFIG.colors.neuralPrism,
        transparent: true,
        opacity: 0.6,
...
interface NeuralPrismMorphAnimationProps {
  className?: string
}
...
export function NeuralPrismMorphAnimation({ className = '' }: NeuralPrismMorphAnimationProps) {
  // Use a unique key to force new canvas creation
  const [canvasKey] = useState(() => `neural-prism-canvas-${Date.now()}`)
...
export default NeuralPrismMorphAnimation
