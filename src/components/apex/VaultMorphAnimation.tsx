...
type Phase = 'idle' | 'morphing_to_vault' | 'showing_vault'

function generateVaultWireframe(maxLines?: number): THREE.Vector3[][] {
  const lines: THREE.Vector3[][] = []
  const edgeCount = VAULTDOOR_WIREFRAME_DATA.length
  const scale = 1.8
  ...
  for (let i = 0; i < targetCount; i++) {
    const edge = VAULTDOOR_WIREFRAME_DATA[i]
...
function SculptureLines() {
  ...
  const targetLines = useMemo(() => generateVaultWireframe(MAX_LINE_COUNT), [])
  ...
  useFrame((state) => {
    ...
    if (phase === 'idle') {
      ...
      if (phaseElapsed > ANIMATION_CONFIG.timing.idleDuration) {
        ...
        setPhase('morphing_to_vault')
        phaseStartRef.current = now
        morphProgressRef.current = 0
      }
    } else if (phase === 'morphing_to_vault') {
      tesseractTimeRef.current += 0.016
    ...
    } else if (phase === 'showing_vault') {
      // Stay showing vault - no transition back
    }
    
    const tesseractTime = tesseractTimeRef.current
    
    const tesseractColor = new THREE.Color(ANIMATION_CONFIG.colors.tesseract)
    const vaultColor = new THREE.Color(ANIMATION_CONFIG.colors.vault)
    
    let targetColor: THREE.Color
    if (phase === 'idle') {
      targetColor = tesseractColor
    } else if (phase === 'morphing_to_vault') {
      targetColor = tesseractColor.clone().lerp(vaultColor, morphProgressRef.current)
    } else {
  ...
  const lineObjects = useMemo(() => {
    ...
    for (let i = 0; i < MAX_LINE_COUNT; i++) {
      ...
      const material = new THREE.LineBasicMaterial({
        color: ANIMATION_CONFIG.colors.vault,
        transparent: true,
        opacity: 0.6,
...
interface VaultMorphAnimationProps {
  className?: string
}
...
export default VaultMorphAnimation
