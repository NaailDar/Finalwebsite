...
export type TesseractVariant = "alice" | "solutions" | "infrastructure" | "security" | "insights";
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (let i = edges.length; i < allEdges.length; i++) {
        ...
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ctx.strokeStyle = `rgba(67, 163, 153, ${opacity})`;
        ctx.lineWidth = 0.5;
        ctx.stroke();
      }
      ...
      for (const [px, py, d] of projected) {
        ...
        ctx.fillStyle = `rgba(67, 163, 153, ${opacity})`;
        ctx.fill();
      }
